...
def interactive():
  quit = False
  while not quit:
    path = input("Enter the path to the image or folder to run, or \'quit\' to exit.")
    if path == 'quit':
      break
...
def main(_):
  ...
  try:
    ...
    bad_data = True
  if bad_data:
    print("Invalid argument, please provide example path(s) with v, or interactive mode with i.")
  else:
    global net
    net = Deploy_Network()
    paths = sys.argv[1:]
